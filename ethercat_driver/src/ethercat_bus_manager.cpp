...
bool EthercatBusManager::setupMaster()
{
  ...
  options.master_id = bus_config_.master_id;
  options.userspace_node_id = bus_config_.userspace_node_id;
  options.create_userspace_master = bus_config_.create_userspace_master;
  if (bus_config_.wait_for_slaves) {
    options.wait_for_slave_count = bus_config_.wait_for_slave_count;
  }
...
void EthercatBusManager::cleanup()
{
  const std::lock_guard<std::mutex> lock(ec_mutex_);
  if (master_) {
    if (activated_) {
